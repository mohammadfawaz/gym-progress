...
#[derive(Deserialize)] struct Auth { #[serde(default)] access_token: Option<String>, #[serde(default)] session: Option<AuthSession>, user: User }
#[derive(Deserialize)] struct AuthSession { #[serde(default)] access_token: Option<String> }
...
fn app() -> Html {
    ...
    let submit_auth = { let email=email.clone(); let password=password.clone(); let signup=signup.clone(); let token=token.clone(); let uid=uid.clone(); let workouts=workouts.clone(); let status=status.clone(); Callback::from(move |e:SubmitEvent| { e.prevent_default(); let email=(*email).clone(); let password=(*password).clone(); let is_signup=*signup; let token=token.clone(); let uid=uid.clone(); let workouts=workouts.clone(); let status=status.clone(); spawn_local(async move { status.set("Connecting…".into()); match auth(&email,&password,is_signup).await { Ok(a)=>{ let Some(access_token) = auth_token(&a) else { status.set("Account created. Check your email to confirm it, then sign in.".into()); return; }; let remote=get_workouts(&access_token,&a.user.id).await.unwrap_or_default(); let local=LocalStorage::get::<Vec<Workout>>(LOCAL).unwrap_or_default(); if remote.is_empty() && !local.is_empty() { for w in &local { let _=put_workout(&access_token,&a.user.id,w).await; } workouts.set(local); status.set("Existing workouts migrated and synced.".into()); } else { workouts.set(remote); status.set("Synced with Supabase.".into()); } token.set(Some(access_token)); uid.set(Some(a.user.id)); } Err(e)=>status.set(e), } }); }) };
